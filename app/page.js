...
export default function Home() {
  ...
  async function saveCompetition(f){
    ...
    if(!data.name){setMsg('Competition name is required.');return;}
    if(f.competitionId){
      const {error}=await supabase.from('competitions').update(data).eq('id',f.competitionId);
      if(error){setMsg(error.message);return;}
      const {data:updated,error:readError}=await supabase.from('competitions').select('*').eq('id',f.competitionId).single();
      if(readError){setMsg(readError.message);return;}
      setModal(null);
      setSelected(updated);
    ...
    if(error){setMsg(error.message);return;}
    setModal(null);
    setCompetitions(prev=>[...prev,created].sort((a,b)=>String(a.start_date||'').localeCompare(String(b.start_date||''))));
    load(created);
  }
  ...
  <div className="layout"><aside><b>Competitions</b><button className="primary createCompetition" onClick={()=>setModal({type:'competition'})}>＋ Create competition</button>{competitions.map(c=><button className={selected?.id===c.id?'sel':''} key={c.id} onClick={()=>load(c)}>{c.name}<small>{c.start_date||'Date TBC'} · {c.venue||''}</small></button>)}</aside>
...
function CompetitionModal({c,close,save}){
  const[f,setF]=useState({
    competitionId:c?.id||'',name:c?.name||'',venue:c?.venue||'',start_date:c?.start_date||'',
