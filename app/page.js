...
export default function Home() {
  ...
  <section className="content"><div className="hero"><div><h2>{selected.name}</h2><p>{selected.venue} · {selected.start_date||'Date TBC'}</p></div><div className="stats"><b>{players.length} players</b><b>{checked} checked in</b><b>{tables.length} tables</b></div></div>
  ...
  <Panel title="Tables" add={()=>setModal({type:'table'})} addText="＋ Add table">
    ...
    {tables.map(t=><div className="row" key={t.id}><div><b>Table {t.table_number} {t.is_accessible?'♿':''}</b><small>{t.table_type||'Standard'} · {t.status||'available'}{t.notes?` · ${t.notes}`:''}</small></div><div className="actions"><select value={t.status||'available'} onChange={e=>saveTable({...t,status:e.target.value},t)}><option value="available">Available</option><option value="occupied">Occupied</option><option value="unavailable">Unavailable</option></select><button onClick={()=>setModal({type:'table',t})}>✏️ Edit</button><a className="scoreLink" href={`/score/${t.scoring_token||''}`} target="_blank" rel="noreferrer">📱 Scoring</a><button className="danger" onClick={()=>delTable(t)}>🗑️ Delete</button><div className="qr">QR<br/><small>Scan for scoring</small></div></div></div>)}
  </Panel>
...
function QRModal({data,close}){
  return <Modal title={`Table ${data.table.table_number} scoring`} close={close}>
    <div style={{textAlign:'center'}}>
      <img src={data.image} alt={`QR code for Table ${data.table.table_number}`} style={{width:260,height:260,maxWidth:'100%'}}/>
      <h3>Scan to score Table {data.table.table_number}</h3>
