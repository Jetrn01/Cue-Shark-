...
export default function Home() {
  ...
  async function createCompetition(f){
    const data={
      name:f.name.trim(),
      venue:f.venue.trim(),
  ...
  {modal?.type==='competition'&&<CompetitionModal c={modal.c} close={()=>setModal(null)} save={modal.c?saveCompetition:createCompetition}/>}
  </>;
}
...
function CompetitionModal({c,close,save}){
  ...
  return <Modal title={c?'Edit competition':'Create competition'} close={close}>
    <form onSubmit={e=>{e.preventDefault();save(f)}}>
      ...
      <div className="ma"><button type="button" onClick={close}>Cancel</button><button className="primary">{c?'Save competition':'Create competition'}</button></div>
    </form>
  </Modal>
}
