...
<div className="tournamentStatusStrip">
  ...
  <Panel title="Matches & Table Assignment">
    <div className="drawTools">
      ...
      {matches.some(m=>m.group_name)&&<>{(selected.format||'').toLowerCase()==='groups → knockout'?<button onClick={()=>generateGroupKnockout(drawSettings)} disabled={matches.some(m=>Number(m.round_number)>1 && !m.group_name)}>🏆 Generate {Math.max(2, Number(drawSettings.group_count||4))*Math.max(1,Number(drawSettings.qualifiers_per_group||4))===16?'Round of 16':'Knockout'} from qualifiers</button>:<button onClick={generateReverseCrossover} disabled={matches.some(m=>Number(m.round_number)>1 && !m.group_name)}>🏆 Generate Reverse Crossover</button>}</>}
