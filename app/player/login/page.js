...
export default function PlayerLogin(){
 ...
 const [msg,setMsg]=useState(''),[busy,setBusy]=useState(false),[matches,setMatches]=useState([]),[competitions,setCompetitions]=useState([]),[tables,setTables]=useState([]),[people,setPeople]=useState({}),[myClubs,setMyClubs]=useState([]),[clubAdd,setClubAdd]=useState('');
 ...
 async function loadPlayer(){
  ...
  await loadMyClubs(data?.[0]?.id||data?.id);
  await loadMatches(data?.[0]?.id||data?.id);
 }


 async function loadMyClubs(pid){if(!pid)return;const {data,error}=await supabase.from('player_clubs').select('club_id,is_primary,status,clubs(id,name,city,region)').eq('player_id',pid).order('is_primary',{ascending:false});if(!error)setMyClubs(data||[]);}
 async function addClub(){if(!clubAdd)return;setBusy(true);const {error}=await supabase.rpc('add_my_club',{p_club_id:clubAdd});if(error)setMsg(error.message);else{setMsg('Club added to your PottersMate profile.');setClubAdd('');await loadMyClubs(player?.id);await loadPlayer();}setBusy(false)}
 async function setPrimaryClub(id){setBusy(true);const {data,error}=await supabase.rpc('set_my_primary_club',{p_club_id:id});if(error)setMsg(error.message);else{setPlayer(data?.[0]||data||player);setMsg('Primary club updated.');await loadMyClubs(player?.id);}setBusy(false)}
 async function removeClub(id){if(!window.confirm('Remove this club from your player profile?'))return;setBusy(true);const {error}=await supabase.rpc('remove_my_club',{p_club_id:id});if(error)setMsg(error.message);else{setMsg('Club removed.');await loadMyClubs(player?.id);}setBusy(false)}
 ...
 <section className="card"><div className="head"><h2>🏠 My clubs</h2></div><p className="muted">Your player identity can belong to more than one club.</p>{myClubs.map(x=><div className="comp" key={x.club_id}><div><b>{x.clubs?.name||'Club'}</b><small>{[x.clubs?.city,x.clubs?.region].filter(Boolean).join(' · ')||'Location not set'}{x.is_primary?' · Primary club':''}</small></div><div className="actions">{!x.is_primary&&<button onClick={()=>setPrimaryClub(x.club_id)}>Make primary</button>}<button onClick={()=>removeClub(x.club_id)}>Remove</button></div></div>)}<div className="clubAddRow"><select value={clubAdd} onChange={e=>setClubAdd(e.target.value)}><option value="">Add another club…</option>{clubs.filter(c=>!myClubs.some(x=>x.club_id===c.id)).map(c=><option key={c.id} value={c.id}>{c.name}{c.city?` · ${c.city}`:''}</option>)}</select><button className="primary" disabled={!clubAdd||busy} onClick={addClub}>＋ Add club</button></div></section><section className="card"><h2>🏆 My competitions</h2>{competitions.length===0?<p className="muted">You haven't played any recorded competitions yet.</p>:competitions.map(c=><div className="comp" key={c.id}><div><b>{c.name}</b><small>{c.venue||''}{c.start_date?` · ${c.start_date}`:''}</small></div><a href={`/tournament/${c.id}`}>View tournament →</a></div>)}</section>
