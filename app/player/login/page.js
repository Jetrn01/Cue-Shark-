...
export default function PlayerLogin(){
 ...
 const [email,setEmail]=useState(''),[password,setPassword]=useState(''),[first,setFirst]=useState(''),[last,setLast]=useState('');
 ...
 useEffect(()=>{if(session) loadPlayer()},[session]);
 ...
 async function submit(e){
  ...
  try{
   if(mode==='signup'){
    ...
    const {data,error}=await supabase.auth.signUp({email:email.trim(),password,options:{data:{first_name:first.trim(),last_name:last.trim()}}});
    if(error){setMsg(error.message);return}
    if(data.session){
      const r=await supabase.rpc('claim_player_account',{p_first_name:first.trim(),p_last_name:last.trim()});
 ...
 if(!session)return <><style>{css}</style><main className="playerLogin"><div className="loginCard"><Brand/><div className="tag">PLAYER AREA</div><h1>{mode==='login'?'Welcome back':'Create your player account'}</h1><p className="muted">{mode==='login'?'Log in to see your matches, tables and results.':'Register once and use PottersMate without needing a QR code.'}</p>{mode==='signup'&&<><input placeholder="First name" value={first} onChange={e=>setFirst(e.target.value)} required/><input placeholder="Last name" value={last} onChange={e=>setLast(e.target.value)} required/></>}<form onSubmit={submit}><input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} required/><input type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} minLength={6} required/><button className="primary" disabled={busy}>{busy?'Please wait…':mode==='login'?'Log in':'Create player account'}</button></form>{msg&&<div className="message">{msg}</div>}<button className="link" onClick={()=>{setMode(mode==='login'?'signup':'login');setMsg('')}}>{mode==='login'?'New player? Create an account':'Already registered? Log in'}</button><a className="back" href="/">← Organiser login</a></div></main></>;

 return <><style>{css}</style><main className="playerDash"><header><Brand/><div><button onClick={logout}>Log out</button></div></header><section className="hero"><div><span className="tag">MY POTTERSMATE</span><h1>Hi, {displayName(player).split(' ')[0]} 👋</h1><p>See your matches, tables and results in one place.</p></div><div className="stats"><div><b>{played.length}</b><span>Played</span></div><div><b>{wins}</b><span>Wins</span></div><div><b>{losses}</b><span>Losses</span></div></div></section>
