-- PottersMate V6 QR / live scoring — final
create extension if not exists pgcrypto;

alter table tournament_tables add column if not exists table_token text;
alter table tournament_tables alter column table_token set default encode(gen_random_bytes(16), 'hex');
...
begin
  select jsonb_build_object(
    'table_number',t.table_number,'is_accessible',t.is_accessible,
    'match_id',m.id,'match_number',m.match_number,'race_to',m.race_to,
    'status',m.status,'score1',coalesce(m.score1,0),'score2',coalesce(m.score2,0),
  ...
  left join lateral (
    select cm.* from competition_matches cm
    where cm.table_id=t.id and cm.status <> 'completed'
    order by cm.match_number limit 1
  ) m on true
...
returns jsonb language plpgsql security definer set search_path=public as $$
declare mid uuid; next1 integer; next2 integer; next_status text;
begin
  if p_player not in (1,2) then raise exception 'Invalid player selection'; end if;
  select m.id into mid from tournament_tables t
  join competition_matches m on m.table_id=t.id
  where t.table_token=p_token and m.status <> 'completed'
  order by m.match_number limit 1;
  if mid is null then raise exception 'No current match is assigned to this table'; end if;
  select coalesce(score1,0),coalesce(score2,0) into next1,next2
  from competition_matches where id=mid for update;
  ...
  into next_status from competition_matches where id=mid;
  update competition_matches set score1=next1,score2=next2,status=next_status where id=mid;
